...
static void cbuf_memcpy(unsigned char *tar, const unsigned char *src, u32 len)
{
	if(len==0)
		return;
	while(len--)
...
u32 get_fifo_size(struct cbuf_t *cbuf)
{
	u32 len = 0;
	len = cbuf->in - cbuf->out;
	return len;
}
...
u32 put_fifo(struct cbuf_t *cbuf, const u8* data_in, u32 len)
{
    ...
    cbuf_memcpy(cbuf->buffer + (cbuf->in&(cbuf->size-1)) , data_in, L);
    cbuf_memcpy(cbuf->buffer, data_in+L, len-L);
    cbuf->in += len;
...
u32 get_fifo(struct cbuf_t *cbuf,  u8* data_out, u32 len)
{
    ...
    cbuf_memcpy(data_out , cbuf->buffer + (cbuf->out &(cbuf->size - 1)), L );
    cbuf_memcpy(data_out+L , cbuf->buffer , len -L);
    cbuf->out += len;
...
u32 get_fifo_no_remove(struct cbuf_t *cbuf,  u8* data_out, u32 len)
{
    if((cbuf == NULL)||(data_out == NULL)||(len==0)){
        return 0;
    }
...
u32 get_fifo_roll(struct cbuf_t *cbuf,  u8* data_out, u32 len)
{
    ...
    cbuf_memcpy(data_out , cbuf->buffer + cbuf->out, L );
    cbuf_memcpy(data_out+L , cbuf->buffer , len -L);
    cbuf->out ++;
 ...
 * size:只能是2的几次幂
 */
u32 cbuffer_init(struct cbuf_t *cbuf,u8 *buf, u32 size, int(*lock_fun)(int))
{
    if((size&(size-1)) != 0){
			return 1;
    }
