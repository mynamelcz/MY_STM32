...
#define MAX_ENC_DATA_LEN	64
u8 ENC_TMP_BUF[MAX_ENC_DATA_LEN];
...
static void zeroize( void *v, u32 n ) 
{
...
{
    my_memset( (u8 *)ctx, 0, sizeof( des_context ) );
}
...
{
    my_memset( (u8 *)ctx, 0, sizeof( des3_context ) );
}
                             ...
                             unsigned char *pkey)
{
    ...
    {
        i -= 8;
        my_memcpy(ENC_TMP_BUF,pdata + i,nlen - i);
        my_memset(ENC_TMP_BUF + nlen % 8, ch, (8 - nlen % 8) % 8);
        des_crypt_ecb( &ctx, ENC_TMP_BUF, (pout + i));
    }
                              ...
                              unsigned int klen)
{
    ...
    if(len > nlen)
    {
        ...
        my_memcpy(ENC_TMP_BUF,pdata + i,nlen - i);
        my_memset(ENC_TMP_BUF + nlen % 8, ch, (8 - nlen % 8) % 8);
        des3_crypt_ecb( &ctx3, ENC_TMP_BUF, (pout + i));
                              ...
                              unsigned char *piv)
{
    ...
    {
        len = nlen + 8 - nlen % 8;
        my_memcpy(ENC_TMP_BUF, pdata, nlen);
        des3_crypt_cbc( &ctx, DES_ENCRYPT, len, pivb, ENC_TMP_BUF, (pout));
    }
    else
    {
        des3_crypt_cbc( &ctx, DES_ENCRYPT, nlen, pivb, pdata, (pout));
    }
