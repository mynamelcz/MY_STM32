...
const static unsigned char AesSbox[16*16]=
...
const static unsigned char AesiSbox[16*16]=
{
...
const static unsigned char AesRcon[17*4]=
{
	0x00, 0x00, 0x00, 0x00,
	0x01, 0x00, 0x00, 0x00,
	0x02, 0x00, 0x00, 0x00,
...
static unsigned char w[16*15];
static unsigned char key[32];
int Nr = 10;
int Nb = 4;
int Nk = 10;

static void aes_memcpy(unsigned char *tar, const unsigned char *src, int len)
{
	while(len--)
		*tar++ = *src++;
}

static void aes_memset(unsigned char *buf, unsigned char dat,int len)
{
	while(len--)
		*buf++ = dat;	
}
static void SetNbNkNr(int keySize)
{
	Nb=4;
	if(keySize==Bits128)
	{
...
//密钥移位函数
static unsigned char* RotWord(unsigned char* word,unsigned char *t)
{
...
//密钥字代换函数
static unsigned char* SubWord(unsigned char* word,unsigned char *t)
{
	unsigned char temp[4];
	for(int j=0;j<4;j++)
	{
...
static void KeyExpansion(void)
{
	...
	for(row=Nk;row<4*(Nr+1);row++)
	{
		...
		if(row%Nk==0)
		{
			SubWord(RotWord(temp,temp),temp);   //先移位，再代换，最后和轮常量异或
...
void Aes_Init(int keysize, unsigned char* keyBytes,int loopnum)
{
	...
	Nr=loopnum;
	aes_memcpy(key,keyBytes,keysize);				//字符串拷贝函数，把keyBytes的keysize个字符复制到key中
	KeyExpansion();
}
...
{
	int i,round;
	aes_memset(&State[0][0],0,16);
	for(i=0;i<4*Nb;i++)                        //这里是先写列后写行的，即输入是一列一列的进来的
	{
		State[i%4][i/4]=input[i];					//换成先写行后写列也是可以的，只要在输出时也是这样就可以了
	}
	AddRoundKey(0);
//	BYTE bydd[0x16]={0xDC,0xCD,0xFE,0xEF,0x98,0x89,0xBA,0xAB,0x54,0x45,0x76,0x67,0x10,0x01,0x32,0x23};
//	memcpy(State,bydd,0x10);								//轮密钥加
